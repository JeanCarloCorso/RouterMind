...
.charts{{display:grid;grid-template-columns:1.25fr .75fr;gap:18px}} .chart{{min-height:290px}} .bars{{height:190px;display:flex;align-items:flex-end;gap:8px;padding-top:18px;border-bottom:1px solid var(--border)}} .bar-col{{height:100%;min-width:0;flex:1;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;gap:7px}} .bar{{width:min(32px,80%);min-height:3px;border-radius:6px 6px 2px 2px;background:linear-gradient(180deg,var(--accent),var(--violet))}} .bar-label{{max-width:100%;overflow:hidden;text-overflow:ellipsis;color:var(--muted);font-size:.67rem;white-space:nowrap}} .key-bars{{display:grid;gap:13px}} .key-bar-head{{display:flex;justify-content:space-between;gap:12px;font-size:.85rem}} .track{{height:8px;margin-top:5px;border-radius:99px;background:#080f1d;overflow:hidden}} .fill{{height:100%;min-width:3px;border-radius:inherit;background:linear-gradient(90deg,var(--violet),var(--accent))}}
...
def _dashboard(request: Request, user, revealed_key: str | None = None, message: str = "") -> HTMLResponse:
    ...
    day_max = max(1, max((int(row["requests"]) for row in charts["by_day"]), default=0))
    day_bars = "".join(
        f"<div class='bar-col' title='{int(row['requests'])} requisições, {int(row['tokens'])} tokens'>"
        f"<div class='bar' style='height:{max(3, round(int(row['requests']) / day_max * 100))}%'></div>"
        f"<span class='bar-label'>{html.escape(str(row['label'])[5:])}</span></div>" for row in charts["by_day"]
    ) or "<p class='muted'>Os dados aparecerão após a primeira requisição.</p>"
    key_max = max(1, max((int(row["requests"]) for row in charts["by_key"]), default=0))
    key_bars = "".join(
        f"<div><div class='key-bar-head'><span>{html.escape(row['label'])}</span><strong>{int(row['requests'])}</strong></div>"
        f"<div class='track'><div class='fill' style='width:{max(1, round(int(row['requests']) / key_max * 100))}%'></div></div></div>"
...
<section class='charts full'><div class='card chart'><div class='card-head'><div><p class='eyebrow'>Volume</p><h2>Requisições por dia</h2></div><span class='muted'>14 dias com atividade</span></div><div class='bars'>{day_bars}</div></div><div class='card chart'><div class='card-head'><div><p class='eyebrow'>Origem</p><h2>Uso por chave</h2></div></div><div class='key-bars'>{key_bars}</div></div></section>
