...
.app-header{{display:flex;align-items:center;justify-content:space-between;gap:18px;padding-bottom:20px;border-bottom:1px solid var(--border);margin-bottom:30px}} .app-actions{{display:flex;align-items:center;gap:8px}} .app-actions form button{{margin:0}} .dashboard-head{{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:22px}} .dashboard-head h1{{margin-bottom:6px}} .stats{{display:grid;grid-template-columns:repeat(5,1fr);gap:11px;margin-bottom:20px}} .stat{{padding:16px 17px;border:1px solid var(--border);border-radius:14px;background:rgba(12,19,33,.7)}} .stat span{{display:block;color:var(--muted);font-size:.72rem;text-transform:uppercase;letter-spacing:.08em}} .stat strong{{display:block;margin-top:6px;font-size:1.04rem}}
...
@media(max-width:1000px){{.stats{{grid-template-columns:repeat(2,1fr)}}}} @media(max-width:800px){{main{{margin-top:18px}}.overview{{grid-template-columns:1fr}}.dashboard-grid,.docs-layout{{grid-template-columns:1fr}}.docs-nav{{position:static}}.dashboard-head{{align-items:flex-start;flex-direction:column}}.card{{padding:20px}}td,th{{white-space:nowrap}}.site-nav{{margin-bottom:30px}}.nav-links .nav-link{{display:none}}}} @media(max-width:520px){{.stats{{grid-template-columns:1fr}}}}
...
def _dashboard(request: Request, user, revealed_key: str | None = None, message: str = "") -> HTMLResponse:
    ...
    ) or "<tr><td colspan='8' class='muted'>Nenhuma requisição registrada ainda.</td></tr>"
...
<div class='card full'><div class='card-head'><div><p class='eyebrow'>Credenciais de acesso</p><h2>Chaves RouteMind</h2></div><span class='muted'>{active_count} no total</span></div><div class='table-wrap'><table><thead><tr><th>Nome</th><th>Prefixo</th><th>Status</th><th>Ação</th></tr></thead><tbody>{key_rows}</tbody></table></div></div>
<div class='card full'><div class='card-head'><div><p class='eyebrow'>Uso recente</p><h2>Histórico de requisições</h2></div><span class='muted'>Últimas 25</span></div><div class='table-wrap'><table><thead><tr><th>Data</th><th>Resultado</th><th>Modelo</th><th>HTTP</th><th>Entrada</th><th>Saída</th><th>Total</th><th>Tempo</th></tr></thead><tbody>{request_history}</tbody></table></div></div></div>"""
